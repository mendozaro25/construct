    ...
    <div class="page-header">
        <div class="page-leftheader">
            ...
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active" aria-current="page"><a href="<?= base_url() ?>umedidas/umedida"><?= $uri["title"] ?></a></li>
            </ol>
        </div>
        <div class="page-rightheader">
            <div class="btn btn-list">
                ...
                <button type="button" class="btn btn-success" onclick="openExcelUploadModal()">
                    <i class="fe fe-upload mr-1"></i>Importar Unidades Medidas
                </button>
            </div>
        </div>
    </div>
        ...
        table1 = $('#table1').DataTable( {
            ...
            "createdRow": function (row, data, index) {
                ...
                if (status === '<?= RECORD_STATUS_ACTIVE_TEXT ?>') {
                    $(row).find('td:eq(3)').html('<span class="mb-0 text-success fs-13 font-weight-semibold">ACTIVO</span>');
                } else if (status === '<?= RECORD_STATUS_INACTIVE_TEXT ?>') {
                    $(row).find('td:eq(3)').html('<span class="mb-0 text-danger fs-13 font-weight-semibold">INACTIVO</span>');
                }
            },        
            language: {
